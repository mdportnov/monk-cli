...
fn matches_process(
    ...
    match app.kind {
        ...
        AppKind::DesktopEntry => {
            // Sandboxed (Flatpak/Snap) apps run as a binary inside the sandbox,
            // not as `exec_path` (the launcher). Match them by cgroup via the
            // parsed application id; exe/name comparison can't see them.
            #[cfg(target_os = "linux")]
            if app.sandbox_id.is_some() {
...
/// Match a sandboxed (Flatpak/Snap) process by reading its cgroup membership.
///
/// `app.sandbox_id` holds the application id parsed from the `.desktop` `Exec`
/// line (e.g. `org.mozilla.firefox`). Systemd places Flatpak apps in
/// `app-flatpak-<id>-<pid>.scope` and Snap apps in `snap.<name>.<app>.scope`,
/// so a substring check against the id reliably catches the app and all of its
/// helper processes.
#[cfg(target_os = "linux")]
fn matches_sandboxed_app(app: &InstalledApp, pid: sysinfo::Pid) -> bool {
    let Some(id) = app.sandbox_id.as_deref() else {
        return false;
    };
    let cgroup_path = format!("/proc/{}/cgroup", pid.as_u32());
    match std::fs::read_to_string(&cgroup_path) {
        Ok(cgroup) => {
            cgroup.contains(&format!("app-flatpak-{id}")) || cgroup.contains(&format!("snap.{id}"))
        }
        // Process exited between enumeration and the cgroup read — not a match.
        Err(_) => false,
    }
}
